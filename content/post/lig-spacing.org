...
Emacs allows ~text-properties~ to associated with strings. A property can be
anything, while some property names tell Emacs to handle the text in a
...
- ~display~ : How Emacs displays for that string, can be any string.
